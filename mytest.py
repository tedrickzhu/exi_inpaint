...
from deepfill.deepfill import deepfill_inpaint

# os.environ['CUDA_VISIBLE_DEVICES']=str(np.argmax([int(x.split()[2]) for x in subprocess.Popen(
#         "nvidia-smi -q -d Memory | grep -A4 GPU | grep Free", shell=True, stdout=subprocess.PIPE).stdout.readlines()]
#         ))

def exifill_inp(imgpath,config,status=0,masklocs=None):

    if not os.path.exists(config.test_dir):
        os.mkdir(config.test_dir)
    basedir = os.path.join(config.test_dir,'pre_inp')
    if not os.path.join(basedir):
        os.mkdir(basedir)
    preinpimgpath = os.path.join(basedir,time.strftime('%Y%m%d%H%M%S')+'_'+os.path.basename(imgpath)+'.png')
    # deepfill_inpaint(imgpath, masklocs,1, checkpointdir, inputimgpath,outputpath)
